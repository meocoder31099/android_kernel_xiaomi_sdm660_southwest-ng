...
{
    if (!handler) {
        pr_debug("feature: register handler is NULL\n");
        return -EINVAL;
    }

    if (handler->feature_id >= KSU_FEATURE_MAX) {
        pr_debug("feature: invalid feature_id %u\n", handler->feature_id);
        return -EINVAL;
    }

    if (!handler->get_handler && !handler->set_handler) {
        pr_debug("feature: no handler provided for feature %u\n", handler->feature_id);
        return -EINVAL;
    }
...
int ksu_get_feature(u32 feature_id, u64 *value, bool *supported)
{
    ...
    if (!value || !supported) {
        pr_debug("feature: invalid parameters\n");
        return -EINVAL;
    }
    ...
    *supported = true;
    ...
    if (ret) {
        pr_debug("feature: get_handler for %u failed: %d\n", feature_id, ret);
    }
...
int ksu_set_feature(u32 feature_id, u64 value)
{
    ...
    if (!handler) {
        pr_debug("feature: feature %u not registered\n", feature_id);
        ret = -EOPNOTSUPP;
        goto out;
    }
    ...
    if (ret) {
        pr_debug("feature: set_handler for %u failed: %d\n", feature_id, ret);
    }
