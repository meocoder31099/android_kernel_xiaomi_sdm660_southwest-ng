...
static int camera_check_event_status(struct v4l2_event *event)
{
	...
	if (event_data->status > MSM_CAMERA_ERR_EVT_BASE) {
		pr_debug("%s : event_data status out of bounds\n",
				__func__);
		pr_debug("%s : Line %d event_data->status 0X%x\n",
...
static int camera_v4l2_vb2_q_init(struct file *filep)
{
	...
	if (!q->drv_priv) {
		pr_debug("%s : memory not available\n", __func__);
		return -ENOMEM;
	}
...
static int camera_v4l2_open(struct file *filep)
{
	...
	if (rc < 0) {
		pr_debug("%s : camera_v4l2_fh_open failed Line %d rc %d\n",
	...
	if (rc < 0) {
		pr_debug("%s : vb2 queue init fails Line %d rc %d\n",
				__func__, __LINE__, rc);
		goto vb2_q_fail;
	}
	...
	if (!atomic_read(&pvdev->opened)) {
		...
		if (rc < 0) {
			pr_debug("%s : session creation failed Line %d rc %d\n",
					__func__, __LINE__, rc);
			goto session_fail;
		}
		...
		if (rc < 0) {
			pr_debug("%s : creation of command_ack queue failed\n",
					__func__);
		...
		if (msm_is_daemon_present()) {
			...
			rc = msm_post_event(&event, MSM_POST_EVT_TIMEOUT);
			if (rc < 0) {
				pr_debug("%s : NEW_SESSION event failed,rc %d\n",
					__func__, rc);
				goto post_fail;
			}
		...
		rc = msm_create_command_ack_q(pvdev->vdev->num,
			find_first_zero_bit((const unsigned long *)&opn_idx,
				MSM_CAMERA_STREAM_CNT_BITS));
		if (rc < 0) {
			pr_debug("%s : creation of command_ack queue failed Line %d rc %d\n",
					__func__, __LINE__, rc);
			goto stream_fail;
		}
	}
