	...
	int size)
{
	...
	if (!addr || (size <= 0) || !dump_data) {
		pr_debug("%s: addr=%pK data=%pK size=%d\n", __func__,
			addr, dump_data, size);
		return;
	}
	for (i = 0; i < (size / u); i++)
		...
		int num_vreg_seq, struct regulator **reg_ptr, int config)
{
	...
	if (num_vreg_seq > num_vreg) {
		pr_debug("%s:%d vreg sequence invalid\n", __func__, __LINE__);
		return -EINVAL;
	}
	...
	if (config) {
		for (i = 0; i < num_vreg_seq; i++) {
			...
			if (regulator_count_voltages(reg_ptr[j]) > 0) {
				...
				if (rc < 0) {
					pr_debug("%s: %s set voltage failed\n",
						__func__,
						curr_vreg->reg_name);
	...
	if (enable) {
		for (i = 0; i < num_vreg_seq; i++) {
			if (vreg_seq) {
				...
				j = i;
			if (IS_ERR_OR_NULL(reg_ptr[j])) {
				pr_debug("%s: %s null regulator\n",
					__func__, cam_vreg[j].reg_name);
				goto disable_vreg;
			}
			rc = regulator_enable(reg_ptr[j]);
			if (rc < 0) {
				pr_debug("%s: %s enable failed\n",
					__func__, cam_vreg[j].reg_name);
				goto disable_vreg;
			}
		...
		enum msm_bus_perf_setting perf_setting)
{
	...
	if (!bus_perf_client) {
		pr_debug("%s: Bus Client NOT Registered!!!\n", __func__);
		return;
	}
	switch (perf_setting) {
	case S_EXIT:
	...
	struct camera_vreg_t *cam_vreg, struct regulator **reg_ptr, int config)
{
	...
	if (!dev || !cam_vreg || !reg_ptr) {
		pr_debug("%s: get failed NULL parameter\n", __func__);
		goto vreg_get_fail;
	}
	if (cam_vreg->type == VREG_TYPE_CUSTOM) {
		if (cam_vreg->custom_vreg_name == NULL) {
			pr_debug("%s : can't find sub reg name\n",
				__func__);
			goto vreg_get_fail;
		}
		...
		if (cam_vreg->reg_name == NULL) {
			pr_debug("%s : can't find reg name\n", __func__);
			goto vreg_get_fail;
		}
	...
	if (config) {
		...
		*reg_ptr = regulator_get(dev, vreg_name);
		...
		if (rc < 0) {
			pr_debug("%s: %s regulator_enable failed\n", __func__,
				vreg_name);
			goto vreg_unconfig;
		}
	} else {
	...
	int gpio_en)
{
	...
	if (!gpio_tbl || !size) {
		pr_debug("%s:%d invalid gpio_tbl %pK / size %d\n", __func__,
	...
	unsigned int *size)
{
	...
	if (!of_node || !dt_prop_name || !size || !reg_s) {
		pr_debug("%s: Error invalid args %pK:%pK:%pK:%pK\n",
	...
	if (!cnt || (cnt % 8)) {
		pr_debug("%s: Error invalid number of entries cnt=%d\n",
			__func__, cnt);
		return -EINVAL;
	}
	cnt /= 4;
		...
		*reg_s = kcalloc(cnt, sizeof(uint32_t),
		...
		ret = of_property_read_u32_array(of_node,
				...
				*reg_s,
				cnt);
		if (ret < 0) {
			pr_debug("%s: No dt reg info read for %s ret=%d\n",
	...
	unsigned int size)
{
	...
	if (!reg_s || !base || !size) {
		pr_debug("%s: Error invalid args\n", __func__);
		return -EINVAL;
	}
	rc = msm_camera_io_w_reg_block((const u32 *) reg_s,
		base, size);
	if (rc < 0)
		pr_debug("%s: Failed dt reg setting write\n", __func__);
	return rc;
}
