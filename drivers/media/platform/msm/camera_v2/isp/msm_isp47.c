			...
			struct msm_isp_ahb_clk_cfg *ahb_cfg)
{
	...
	if (vote && vfe_dev->ahb_vote != vote) {
		...
		if (rc)
			pr_debug("%s: failed to set ahb vote to %x\n",
				__func__, vote);
		else
	...
	struct vfe_device *vfe_dev)
{
	...
	if (violation_status > 39) {
		pr_debug("%s: invalid violation status %d\n",
...
void msm_vfe47_process_error_status(struct vfe_device *vfe_dev)
{
	...
	if (error_status1 & (1 << 16))
		pr_debug("%s: status hdr be bus overflow\n", __func__);
	...
	if (error_status1 & (1 << 19))
		pr_debug("%s: status hdr bhist bus overflow\n", __func__);
	...
	if (error_status1 & (1 << 24))
		pr_debug("%s: status aec bg bus overflow\n", __func__);
	if (error_status1 & (1 << 25))
		pr_debug("%s: status dsp error\n", __func__);
}
	...
	void *arg)
{
	...
	if (!fe_cfg->offline_mode) {
		...
		if (rc < 0 || !buf) {
			pr_debug("%s: No fetch buffer rc= %d buf= %pK\n",
				__func__, rc, buf);
	...
	struct msm_vfe_testgen_cfg *testgen_cfg)
{
	...
	case ISP_BAYER_GBGBGB:
		...
		break;
	default:
		pr_debug("%s: invalid pix pattern %d\n",
			__func__, bit_per_pixel);
		break;
	}
	...
	uint8_t is_stream_on)
{
	...
	if (vfe_dev->buf_mgr->secure_enable == SECURE_MODE) {
		pr_debug("%s: Cannot configure hvx, secure_mode: %d\n",
			__func__,
			vfe_dev->buf_mgr->secure_enable);
		return;
	}
	if (!vfe_dev->hvx_clk) {
		pr_debug("%s: no stream_clk\n", __func__);
		return;
	}
	if (is_stream_on) {
		/* Enable HVX */
		if (!vfe_dev->hvx_clk_state) {
			...
			if (rc) {
				pr_debug("%s: stream_clk enable failed\n",
						__func__);
				return;
			}
		...
		rc = msm_camera_clk_enable(&vfe_dev->pdev->dev,
			...
			vfe_dev->num_hvx_clk, is_stream_on);
		if (rc) {
			pr_debug("%s: stream_clk disable failed\n",
					__func__);
			return;
		}
	...
	struct msm_vfe_stats_stream *stream_info)
{
	...
	case STATS_COMP_IDX_BHIST:
		...
		break;
	default:
		pr_debug("%s: Invalid stats idx %d\n", __func__,
...
void msm_vfe47_stats_cfg_ub(struct vfe_device *vfe_dev)
{
	...
	if (vfe_dev->pdev->id == ISP_VFE1)
		...
		pr_debug("%s: incorrect VFE device\n", __func__);
	...
	struct msm_isp_bandwidth_mgr *isp_bandwidth_mgr)
{
	...
	if (!isp_bandwidth_mgr->bus_client) {
		pr_debug("%s: client register failed\n", __func__);
		return -EINVAL;
	}
...
int msm_vfe47_get_max_clk_rate(struct vfe_device *vfe_dev, long *rate)
{
	...
	if (!vfe_dev || !rate) {
		pr_debug("%s:%d failed: vfe_dev %pK rate %pK\n", __func__,
	...
	*rate = 0;
	if (!vfe_dev->hw_info) {
		pr_debug("%s:%d failed: vfe_dev->hw_info %pK\n", __func__,
	...
	if (clk_idx >= vfe_dev->num_clk) {
		pr_debug("%s:%d failed: clk_idx %d max array size %zd\n",
	...
	if (round_rate < 0) {
		pr_debug("%s: Invalid vfe clock rate\n", __func__);
		return -EINVAL;
	}
	...
	struct msm_isp_clk_rates *rates)
{
	...
	if (!vfe_dev || !rates) {
		pr_debug("%s:%d failed: vfe_dev %pK rates %pK\n", __func__,
			__LINE__, vfe_dev, rates);
		return -EINVAL;
	}

	if (!vfe_dev->pdev) {
		pr_debug("%s:%d failed: vfe_dev->pdev %pK\n", __func__,
	...
	if (!of_node) {
		pr_debug("%s %d failed: of_node = %pK\n", __func__,
		 __LINE__, of_node);
		return -EINVAL;
	}
...
int msm_vfe47_get_regulators(struct vfe_device *vfe_dev)
{
	...
	for (i = 0; i < vfe_dev->vfe_num_regulators; i++) {
		...
		if (IS_ERR(vfe_dev->regulator_info[i].vdd)) {
			pr_debug("%s: Regulator vfe get failed %ld\n", __func__,
