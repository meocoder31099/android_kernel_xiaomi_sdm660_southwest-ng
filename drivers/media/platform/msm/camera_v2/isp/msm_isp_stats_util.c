	...
	struct msm_vfe_stats_stream *stream_info, uint32_t pingpong_status)
{
	...
	if (stats_idx >= vfe_dev->hw_info->stats_hw_info->num_stats_type ||
		...
		pr_debug("%s Invalid stats index %d\n", __func__, stats_idx);
		return -EINVAL;
	}
	stats_pingpong_offset =
	...
	uint32_t *comp_stats_type_mask, uint32_t pingpong_status)
{
	...
	if (!vfe_dev || !ts || !buf_event || !stream_info) {
		pr_debug("%s:%d failed: invalid params %pK %pK %pK %pK\n",
	...
	uint32_t pingpong_status, bool is_composite)
{
	...
	for (i = 0; i < vfe_dev->hw_info->stats_hw_info->num_stats_type; i++) {
		...
		rc = msm_isp_stats_buf_divert(vfe_dev, ts,
				...
				pingpong_status);
		if (rc < 0) {
			pr_debug("%s:%d failed: stats buf divert rc %d\n",
	...
	struct msm_vfe_stats_stream *stream_info)
{
	...
	if (!(vfe_dev->hw_info->stats_hw_info->stats_capability_mask &
		...
		pr_debug("%s: Stats type not supported\n", __func__);
		return rc;
	}

	if (stream_info->state != STATS_AVAILABLE) {
		pr_debug("%s: Stats already requested\n", __func__);
		return rc;
	}

	if (stream_req_cmd->framedrop_pattern >= MAX_SKIP) {
		pr_debug("%s: Invalid framedrop pattern\n", __func__);
		return rc;
	}

	if (stream_req_cmd->irq_subsample_pattern >= MAX_SKIP) {
		pr_debug("%s: Invalid irq subsample pattern\n", __func__);
		return rc;
	}

	if (stream_req_cmd->composite_flag >
		vfe_dev->hw_info->stats_hw_info->num_stats_comp_mask) {
		pr_debug("%s: comp grp %d exceed max %d\n",
	...
	if (stream_info->num_isp == 0) {
		...
		if (stream_info->vfe_mask & (1 << vfe_dev->pdev->id)) {
			pr_debug("%s: stats %d already requested for vfe %d\n",
		...
		if (stream_info->framedrop_period != framedrop_period)
			rc = -EINVAL;
		if (rc) {
			pr_debug("%s: Stats stream param mismatch between vfe\n",
				__func__);
			return rc;
		}
	}
...
int msm_isp_release_stats_stream(struct vfe_device *vfe_dev, void *arg)
{
	...
	if (vfe_idx == -ENOTTY || stream_info->stream_handle[vfe_idx] !=
		...
		pr_debug("%s: Invalid stream handle %x, expected %x\n",
	...
	if (stream_info->state == STATS_AVAILABLE) {
		pr_debug("%s: stream already release\n", __func__);
		return rc;
	}
	...
	struct msm_vfe_stats_stream *stream_info)
{
	...
	if (stream_info->bufq_handle == 0) {
		pr_debug("%s: no buf configured for stream: 0x%x\n",
		...
		int active)
{
	...
	if (rc <= 0) {
		rc = rc ? rc : -ETIMEDOUT;
		pr_debug("%s: wait for stats stream %x idx %d state %d active %d config failed %d\n",
	...
	struct msm_vfe_stats_stream_cfg_cmd *stream_cfg_cmd)
{
	...
	if (stream_cfg_cmd->num_streams > MSM_ISP_STATS_MAX) {
		pr_debug("%s invalid num_streams %d\n", __func__,
			stream_cfg_cmd->num_streams);
		return -EINVAL;
	}
	...
	struct msm_vfe_stats_stream_cfg_cmd *stream_cfg_cmd)
{
	...
	for (i = 0; i < stream_cfg_cmd->num_streams; i++) {
		...
		if (vfe_idx == -ENOTTY || stream_info->stream_handle[vfe_idx] !=
			...
			pr_debug("%s: Invalid stream handle: 0x%x received expected %x\n",
...
int msm_isp_update_stats_stream(struct vfe_device *vfe_dev, void *arg)
{
	...
	for (i = 0; i < update_cmd->num_streams; i++) {
		...
		if (STATS_IDX(update_info->stream_handle)
			...
			pr_debug("%s: stats idx %d out of bound!\n", __func__,
	...
	for (i = 0; i < update_cmd->num_streams; i++) {
		...
		if (vfe_idx == -ENOTTY || stream_info->stream_handle[vfe_idx] !=
			...
			pr_debug("%s: stats stream handle %x %x mismatch!\n",
