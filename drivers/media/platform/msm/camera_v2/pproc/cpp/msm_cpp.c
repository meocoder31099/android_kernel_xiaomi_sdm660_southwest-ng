	...
	int (*client_vbif_error_handler)(void *, uint32_t))
{
	if (dev == NULL || client >= VBIF_CLIENT_MAX) {
		pr_debug("%s: Fail to register handler! dev = %pK, client %d\n",
			__func__, dev, client);
		return;
	}
...
static int msm_cpp_init_bandwidth_mgr(struct cpp_device *cpp_dev)
{
	...
	if (rc < 0) {
		...
		pr_debug("Fail bus scale update %d\n", rc);
		return -EINVAL;
	}
...
static void msm_cpp_deinit_bandwidth_mgr(struct cpp_device *cpp_dev)
{
	...
	if (rc < 0) {
		pr_debug("Failed to unregister %d\n", rc);
		return;
	}
}
	...
	struct msm_cpp_buffer_info_t *buffer_info)
{
	...
	list_for_each_entry_safe(buff, save, buff_head, entry) {
		if (buff->map_info.buff_info.index == buffer_info->index) {
			pr_debug("error buf index already queued\n");
			pr_debug("error buf, fd %d idx %d native %d ssid %d %d\n",
				buffer_info->fd, buffer_info->index,
				buffer_info->native_buff,
				buff_queue->session_id,
				buff_queue->stream_id);
			pr_debug("existing buf,fd %d idx %d native %d id %x\n",
	...
	if (buff_queue->security_mode == SECURE_MODE)
		...
		rc = cam_smmu_get_phy_addr(cpp_dev->iommu_hdl,
			...
			(size_t *)&buff->map_info.len);
	if (rc < 0) {
		pr_debug("ION mmap for CPP buffer failed\n");
		kfree(buff);
		goto error;
	}
	...
	struct msm_cpp_buffer_map_list_t *buff)
{
	...
	if (buff_queue->security_mode == SECURE_MODE)
		...
		ret = cam_smmu_put_phy_addr(cpp_dev->iommu_hdl,
			buff->map_info.buf_fd);
	if (ret < 0)
		pr_debug("Error: cannot put the iommu handle back to ion fd\n");
	...
	struct msm_cpp_frame_info_t *frame_info)
{
	...
	for (i = 0; i < frame_info->num_strips; i++) {
		pr_debug("stripe %d: in %x, out1 %x out2 %x, out3 %x, out4 %x\n",
		...
		if (tnr_enabled) {
			pr_debug("stripe %d: read_ref %x, write_ref %x\n", i,
		...
		if (cds_en) {
			pr_debug("stripe %d:, dsdn_off %x\n", i,
				cpp_frame_msg[s_base + rd_ref_off + i * s_size]
				);
		}

		if (ubwc_enabled) {
			pr_debug("stripe %d: metadata %x, %x, %x, %x\n", i,
	...
	void *token)
{
	...
	if (cpp_dev->fault_status != CPP_IOMMU_FAULT_NONE) {
		pr_debug("fault already detected %d\n", cpp_dev->fault_status);
		return;
	}
	...
	struct device *dev, unsigned long iova, int flags, void *token)
{
	...
	if (token) {
		...
		if (cpp_dev->fault_status != CPP_IOMMU_FAULT_DETECTED) {
			pr_debug("fault recovery already done %d\n",
				cpp_dev->fault_status);
			return;
		}
		...
		pr_debug("in recovery, HALT status = 0x%x\n",
		...
		pr_debug("counter %d HALT status later = 0x%x\n",
			counter,
			msm_camera_io_r(cpp_dev->cpp_hw_base + 0x10));
		...
		/* MMSS_A_CPP_IRQ_STATUS_0 = 0x10 */
		pr_debug("counter %d after reset IRQ_STATUS_0 = 0x%x\n",
			counter,
			msm_camera_io_r(cpp_dev->cpp_hw_base + 0x10));
		...
		for (i = 0; i < queue_len; i++) {
			if (cpp_timer.data.processed_frame[i]) {
				...
				pr_debug("Fault on identity=0x%x, frame_id=%03d\n",
					processed_frame[i]->identity,
					processed_frame[i]->frame_id);
				pr_debug("ifd %d ofd %d dfd %d t0fd %d t1fd %d\n",
		...
		pr_debug("fault recovery successful\n");
	}
}
...
static int cpp_init_mem(struct cpp_device *cpp_dev)
{
	...
	if (rc < 0) {
		pr_debug("smmu get handle failed\n");
		return -ENODEV;
	}
...
static irqreturn_t msm_cpp_irq(int irq_num, void *data)
{
	...
	if (irq_status & 0x8) {
		...
		if (tx_level < MSM_CPP_TX_FIFO_LEVEL) {
			...
			pr_debug("Fatal invalid tx level %d\n", tx_level);
			goto err;
		}
		spin_lock_irqsave(&cpp_dev->tasklet_lock, flags);
		queue_cmd = &cpp_dev->tasklet_queue_cmd[cpp_dev->taskletq_idx];
		if (queue_cmd->cmd_used) {
			pr_debug("%s:%d] cpp tasklet queue overflow tx %d rc %x\n",
...
static int cpp_init_hardware(struct cpp_device *cpp_dev)
{
	...
	if (rc < 0) {
		pr_debug("%s: failed to enable regulators\n", __func__);
	...
	if (cpp_dev->micro_reset) {
		...
		if (rc < 0) {
			pr_debug("%s: reset micro clk failed\n", __func__);
			goto clk_failed;
		}
	}
	...
	if (cpp_dev->state != CPP_STATE_BOOT) {
		...
		if (rc < 0) {
			pr_debug("buf mngr req ops failed\n");
	...
	if (rc < 0) {
		pr_debug("%s: create buff queue failed with err %d\n",
			__func__, rc);
		goto pwr_collapse_reset;
	}
	...
	if (cpp_dev->fw_name_bin) {
		...
		if (rc < 0) {
			pr_debug("%s: load firmware failure %d-retry\n",
				__func__, rc);
...
static int32_t cpp_load_fw(struct cpp_device *cpp_dev, char *fw_name_bin)
{
	...
	if (rc < 0) {
		pr_debug("%s:%d: failed to vote for AHB\n", __func__, __LINE__);
		goto end;
	}
	...
	if (rc) {
		pr_debug("%s:%d] poll command %x failed %d\n", __func__, __LINE__,
	...
	if (rc) {
		pr_debug("%s:%d] poll rx empty failed %d\n",
	...
	if (rc < 0)
		pr_debug("update cpp gdscr status failed\n");
	...
	if (rc) {
		pr_debug("%s:%d] poll command 0x1 failed %d\n", __func__,
				__LINE__,
			rc);
		goto vote;
	}
...
static int32_t msm_cpp_reset_vbif_and_load_fw(struct cpp_device *cpp_dev)
{
	...
	rc = cpp_load_fw(cpp_dev, cpp_dev->fw_name_bin);
	if (rc < 0)
		pr_debug("Reset and load fw failed %d\n", rc);
...
static int cpp_vbif_error_handler(void *dev, uint32_t vbif_error)
{
	...
	if (dev == NULL || vbif_error >= CPP_VBIF_ERROR_MAX) {
		pr_debug("failed: dev %pK, vbif error %d\n", dev, vbif_error);
		return -EINVAL;
	}
	...
	/* MMSS_A_CPP_IRQ_STATUS_0 = 0x10 */
	pr_debug("%s: before reset halt... read MMSS_A_CPP_IRQ_STATUS_0 = 0x%x\n",
		__func__, msm_camera_io_r(cpp_dev->cpp_hw_base + 0x10));

	pr_debug("%s: start reset bus bridge on FD + CPP!\n", __func__);
	/* MMSS_A_CPP_RST_CMD_0 = 0x8,  firmware reset = 0x3DF77 */
	msm_camera_io_w(0x3DF77, cpp_dev->cpp_hw_base + 0x8);

	/* MMSS_A_CPP_IRQ_STATUS_0 = 0x10 */
	pr_debug("%s: after reset halt... read MMSS_A_CPP_IRQ_STATUS_0 = 0x%x\n",
		__func__, msm_camera_io_r(cpp_dev->cpp_hw_base + 0x10));
...
static int cpp_open_node(struct v4l2_subdev *sd, struct v4l2_subdev_fh *fh)
{
	...
	if (!sd || !fh) {
		pr_debug("Wrong input parameters sd %pK fh %pK!\n",
			sd, fh);
		return -EINVAL;
	}
	cpp_dev = v4l2_get_subdevdata(sd);
	if (!cpp_dev) {
		pr_debug("failed: cpp_dev %pK\n", cpp_dev);
		return -EINVAL;
	}
	...
	if (cpp_dev->cpp_open_cnt == 1) {
		...
		if (rc < 0) {
			pr_debug("Error: init memory fail\n");
			cpp_dev->cpp_open_cnt--;
...
static int cpp_close_node(struct v4l2_subdev *sd, struct v4l2_subdev_fh *fh)
{
	...
	if (!sd) {
		pr_debug("Wrong input sd parameter\n");
		return -EINVAL;
	}
	cpp_dev =  v4l2_get_subdevdata(sd);
	...
	if (cpp_dev->turbo_vote) {
		...
		rc = cam_cx_ipeak_unvote_cx_ipeak(cpp_dev->cx_ipeak_bit);
			if (rc)
				pr_debug("cx_ipeak_update failed\n");
			else
				cpp_dev->turbo_vote = false;
	}
	...
	if (cpp_dev->cpp_open_cnt == 0) {
		...
		if (cpp_dev->iommu_state == CPP_IOMMU_STATE_ATTACHED) {
			...
			if (rc < 0)
				pr_debug("Error: Detach fail in release\n");
...
static void msm_cpp_do_timeout_work(struct work_struct *work)
{
	...
	if (!work || (cpp_timer.data.cpp_dev->state != CPP_STATE_ACTIVE)) {
		pr_debug("Invalid work:%pK or state:%d\n", work,
			cpp_timer.data.cpp_dev->state);
		/* Do not flush queue here as it is not a fatal error */
		goto end;
	}
	if (!atomic_read(&cpp_timer.used)) {
		pr_debug("Delayed trigger, IRQ serviced\n");
		/* Do not flush queue here as it is not a fatal error */
		goto end;
	}
	...
	queue_len = queue->len;
	if (!queue_len) {
		pr_debug("%s:%d: irq serviced after timeout.Ignore timeout\n",
			__func__, __LINE__);
	...
	if (rc) {
		pr_debug("Firmware loading failed-retry\n");
		rc = msm_cpp_reset_vbif_and_load_fw(cpp_dev);
		if (rc < 0) {
			pr_debug("Firmware loading failed\n");
			goto error;
		}
	} else {
	...
	for (i = 0; i < queue_len; i++) {
		...
		if (!processed_frame[i]) {
			pr_debug("process frame null , queue len %d\n",
				queue_len);
	...
	for (i = 0; i < queue_len; i++) {
		...
		rc = msm_cpp_poll_rx_empty(cpp_dev->base);
		if (rc) {
			pr_debug("%s:%d: Reschedule payload failed %d\n",
				__func__, __LINE__, rc);
			goto error;
		}
		...
		for (j = 0; j < cpp_dev->payload_params.stripe_base; j++,
			...
			if (fifo_counter % MSM_CPP_RX_FIFO_LEVEL == 0) {
				rc = msm_cpp_poll_rx_empty(cpp_dev->base);
				if (rc) {
					pr_debug("%s:%d] poll failed %d rc %d\n",
		...
		if (rc) {
			pr_debug("%s:%d: Rescheduling plane info failed %d\n",
				__func__, __LINE__, rc);
			goto error;
		}
		/* send stripes */
		...
		if (rc) {
			pr_debug("%s:%d] Rescheduling stripe info failed %d\n",
				__func__, __LINE__, rc);
			goto error;
		}
		/* send trailer */
		...
		if (fifo_counter % MSM_CPP_RX_FIFO_LEVEL == 0) {
			rc = msm_cpp_poll_rx_empty(cpp_dev->base);
			if (rc) {
				pr_debug("%s:%d] Reschedule trailer failed %d\n",
					__func__, __LINE__, rc);
				goto error;
			}
			fifo_counter = 0;
	...
	struct msm_queue_cmd *frame_qcmd)
{
	...
	if (cpp_dev->processing_q.len < MAX_CPP_PROCESSING_FRAME) {
		...
		rc = msm_cpp_poll_rx_empty(cpp_dev->base);
		if (rc) {
			pr_debug("%s:%d: Scheduling payload failed %d\n",
				__func__, __LINE__, rc);
			goto dequeue_frame;
		}
		msm_cpp_write(0x6, cpp_dev->base);
		...
		if (rc) {
			pr_debug("%s:%d: Scheduling plane info failed %d\n",
				__func__, __LINE__, rc);
			goto dequeue_frame;
		}
		/* send stripes */
		...
		if (rc) {
			pr_debug("%s:%d: Scheduling stripe info failed %d\n",
				__func__, __LINE__, rc);
			goto dequeue_frame;
		}
		/* send trailer */
		if ((fifo_counter % MSM_CPP_RX_FIFO_LEVEL) == 0) {
			rc = msm_cpp_poll_rx_empty(cpp_dev->base);
			if (rc) {
				pr_debug("%s: Scheduling trailer failed %d\n",
				__func__, rc);
				goto dequeue_frame;
			}
			fifo_counter = 0;
	...
	struct msm_camera_v4l2_ioctl_t *ioctl_ptr)
{
	...
	if ((new_frame->msg_len == 0) ||
		...
		pr_debug("%s:%d: Invalid frame len:%d\n", __func__,
	...
	uint32_t num_output_bufs)
{
	...
	do {
		...
		if (!set_group_buffer_len) {
			pr_debug("%s: invalid set group buffer cmd len %d\n",
		...
		if (!set_group_buffer_w_duplication) {
			pr_debug("%s: set group buffer data alloc failed\n",
				__func__);
			rc = -ENOMEM;
			break;
		}
		...
		*ptr++ = 0;
		...
		for (i = 1; i < num_output_bufs; i++) {
			...
			if (!out_phyaddr1) {
				pr_debug("%s: error getting o/p phy addr\n",
					__func__);
				rc = -EINVAL;
				break;
			}
		...
		if (rc < 0) {
			pr_debug("%s: Send Command Error rc %d\n", __func__, rc);
			break;
		}
	...
	uint32_t num_output_bufs)
{
	...
	if (new_frame->duplicate_output) {
		pr_debug("cannot support duplication enable\n");
		rc = -EINVAL;
		goto exit;
	}
	...
	*ptr++ = 0;
	...
	if (rc < 0)
		pr_debug("Send Command Error rc %d\n", rc);
	...
	struct msm_cpp_frame_info_t *new_frame)
{
	...
	if (!new_frame) {
		pr_debug("%s: Frame is Null\n", __func__);
		return -EINVAL;
	}

	if (cpp_dev->state == CPP_STATE_OFF) {
		pr_debug("%s: cpp state is off, return fatal error\n", __func__);
		return -EINVAL;
	}
	...
	if (cpp_frame_msg == NULL ||
		...
		pr_debug("Length is not correct or frame message is missing\n");
		return -EINVAL;
	}

	if (cpp_dev->fault_status == CPP_IOMMU_FAULT_RECOVERED) {
		pr_debug("Error, page fault occurred %d\n",
			cpp_dev->fault_status);
		return -EINVAL;
	} else if (cpp_dev->fault_status == CPP_IOMMU_FAULT_DETECTED) {
		pr_debug("drop frame, page fault occurred %d\n",
			cpp_dev->fault_status);
		return -EFAULT;
	}
	...
	if (cpp_frame_msg[new_frame->msg_len - 1] !=
		MSM_CPP_MSG_ID_TRAILER) {
		pr_debug("Invalid frame message\n");
		return -EINVAL;
	}
	...
	if ((!new_frame->num_strips) ||
		...
		pr_debug("Invalid frame message, #stripes=%d, stripe indices=[%d,%d]\n",
	...
	if (!stripe_size) {
		pr_debug("Invalid frame message, invalid stripe_size (%d)!\n",
			stripe_size);
		return -EINVAL;
	}
	...
	if ((stripe_base == UINT_MAX) ||
		...
		pr_debug("Invalid frame message, num_strips %d is large\n",
			new_frame->num_strips);
		return -EINVAL;
	}
	...
	if ((stripe_base + new_frame->num_strips * stripe_size + 1) !=
		new_frame->msg_len) {
		pr_debug("Invalid frame message,len=%d,expected=%d\n",
			new_frame->msg_len,
			(stripe_base +
	...
	if (cpp_dev->iommu_state != CPP_IOMMU_STATE_ATTACHED) {
		pr_debug("IOMMU is not attached\n");
		return -EAGAIN;
	}
	...
	if (!in_phyaddr) {
		pr_debug("%s: error gettting input physical address\n", __func__);
		rc = -EINVAL;
		goto frame_msg_err;
	}
	...
	if (new_frame->we_disable == 0) {
		...
		if ((new_frame->output_buffer_info[0].native_buff == 0) &&
			...
			num_output_bufs =
				msm_cpp_check_buf_type(&buff_mgr_info,
					new_frame);
			if (!num_output_bufs) {
				pr_debug("%s: error getting buffer %d\n",
		...
		if (!out_phyaddr0) {
			pr_debug("%s: error gettting output physical address\n",
				__func__);
			rc = -EINVAL;
			goto phyaddr_err;
		}
	...
	if (tnr_enabled) {
		...
		if (!tnr_scratch_buffer0) {
			pr_debug("error getting scratch buffer physical address\n");
			rc = -EINVAL;
			goto phyaddr_err;
		}
	...
	rc = msm_cpp_set_group_buffer(cpp_dev, new_frame, out_phyaddr0,
		num_output_bufs);
	if (rc) {
		pr_debug("%s: set group buffer failure %d\n", __func__, rc);
		goto phyaddr_err;
	}
	...
	if (rc < 0) {
		pr_debug("%s: error cannot send frame to hardware\n", __func__);
		rc = -EINVAL;
		goto qcmd_err;
	}
	...
	struct msm_camera_v4l2_ioctl_t *ioctl_ptr)
{
	...
	if (!frame) {
		pr_debug("%s: Error allocating frame\n", __func__);
		rc = -EINVAL;
	} else {
		if ((frame->msg_len == 0) ||
	...
	if (copy_to_user((void __user *)ioctl_ptr->ioctl_ptr,
		...
		pr_debug("Error: cannot copy k_frame_info\n");
		return -EFAULT;
	}
	...
	struct msm_camera_v4l2_ioctl_t *ioctl_ptr)
{
	...
	if ((ioctl_ptr->ioctl_ptr == NULL) || (ioctl_ptr->len == 0)) {
		pr_debug("%s: Wrong ioctl_ptr %pK / len %zu\n", __func__,
	...
	if (is_compat_task()) {
		...
		if (ret)
			pr_debug("Copy from user fail %d\n", ret);
	}
	return ret ? -EFAULT : 0;
}
#else
...
static int32_t msm_cpp_fw_version(struct cpp_device *cpp_dev)
{
	...
	if (rc) {
		pr_debug("%s:%d] poll command 0x2 failed %d\n", __func__,
			__LINE__, rc);
		goto end;
	}
	...
	struct msm_camera_v4l2_ioctl_t **ioctl_ptr)
{
	...
	default: {
		if (ioctl_ptr == NULL) {
			pr_debug("Wrong ioctl_ptr for cmd %u\n", cmd);
			return -EINVAL;
		}
		...
		*ioctl_ptr = arg;
		if (((*ioctl_ptr) == NULL) ||
			...
			((*ioctl_ptr)->len == 0)) {
			pr_debug("Error invalid ioctl argument cmd %u\n", cmd);
			return -EINVAL;
		}
		break;
	}
	...
	unsigned long clock, int idx)
{
	...
	if ((clock >= cpp_dev->hw_info.freq_tbl
		...
		if (ret) {
			pr_debug("cx_ipeak voting failed setting clock below turbo\n");
		...
		if (cpp_dev->turbo_vote) {
			...
			if (ret)
				pr_debug("cx_ipeak unvoting failed\n");
			else
				cpp_dev->turbo_vote = false;
		}
	}
			...
			unsigned int cmd, void *arg)
{
	...
	if (_IOC_DIR(cmd) == _IOC_NONE) {
		pr_debug("Invalid ioctl/subdev cmd %u\n", cmd);
		return -EINVAL;
	}

	rc = msm_cpp_validate_input(cmd, arg, &ioctl_ptr);
	if (rc != 0) {
		pr_debug("input validation failed\n");
		return rc;
	}
	mutex_lock(&cpp_dev->mutex);
	...
	case VIDIOC_MSM_CPP_LOAD_FIRMWARE: {
		...
		if (cpp_dev->is_firmware_loaded == 0) {
			...
			if (ioctl_ptr->ioctl_ptr == NULL) {
				pr_debug("ioctl_ptr->ioctl_ptr=NULL\n");
				kfree(cpp_dev->fw_name_bin);
				cpp_dev->fw_name_bin = NULL;
				mutex_unlock(&cpp_dev->mutex);
				return -EINVAL;
			...
			*(cpp_dev->fw_name_bin+ioctl_ptr->len) = '\0';
			...
			if (rc < 0) {
				pr_debug("%s: get firmware failure %d\n",
					__func__, rc);
				enable_irq(cpp_dev->irq->start);
				mutex_unlock(&cpp_dev->mutex);
				return rc;
	...
	case VIDIOC_MSM_CPP_ENQUEUE_STREAM_BUFF_INFO: {
		...
		if (k_stream_buff_info.num_buffs > MSM_CAMERA_MAX_STREAM_BUF) {
			pr_debug("%s:%d: unexpected large num buff requested\n",
				__func__, __LINE__);
			kfree(u_stream_buff_info);
			mutex_unlock(&cpp_dev->mutex);
			return -EINVAL;
		...
		buff_queue_info = msm_cpp_get_buff_queue_entry(cpp_dev,
			...
			(k_stream_buff_info.identity & 0xFFFF));
		if (buff_queue_info == NULL) {
			pr_debug("error finding buffer queue entry identity:%d\n",
...
STREAM_BUFF_END:
	...
	case VIDIOC_MSM_CPP_GET_EVENTPAYLOAD: {
		...
		if (!event_qcmd) {
			pr_debug("no queue cmd available\n");
			mutex_unlock(&cpp_dev->mutex);
			return -EINVAL;
		}
	...
	case VIDIOC_MSM_CPP_SET_CLOCK: {
		...
		if (ioctl_ptr->len != sizeof(struct msm_cpp_clock_settings_t)) {
			pr_debug("Not valid ioctl_ptr->len\n");
			mutex_unlock(&cpp_dev->mutex);
			return -EINVAL;
		}
		...
		if (clock_settings.clock_rate > 0) {
			...
			if (msm_cpp_core_clk_idx < 0) {
				pr_debug(" Fail to get clock index\n");
				mutex_unlock(&cpp_dev->mutex);
				return -EINVAL;
			}
			rc = msm_cpp_update_bandwidth_setting(cpp_dev,
			...
			if (rc < 0) {
				pr_debug("Fail to set core clk\n");
				mutex_unlock(&cpp_dev->mutex);
				return -EINVAL;
			}
			if (clock_rate != clock_settings.clock_rate)
				pr_debug("clock rate differ from settings\n");
	...
	case MSM_SD_SHUTDOWN:
		...
		mutex_unlock(&cpp_dev->mutex);
		pr_debug("shutdown cpp node. open cnt:%d\n",
			cpp_dev->cpp_open_cnt);
	...
	case VIDIOC_MSM_CPP_QUEUE_BUF: {
		...
		if (ioctl_ptr->len != sizeof(struct msm_pproc_queue_buf_info)) {
			pr_debug("%s: Not valid ioctl_ptr->len\n", __func__);
			mutex_unlock(&cpp_dev->mutex);
			return -EINVAL;
		}
	...
	case VIDIOC_MSM_CPP_IOMMU_ATTACH: {
		if (cpp_dev->iommu_state == CPP_IOMMU_STATE_DETACHED) {
			...
			rc = msm_cpp_copy_from_ioctl_ptr(&cpp_attach_info,
				ioctl_ptr);
			if (rc < 0) {
				pr_debug("CPP_IOMMU_ATTACH copy from user fail\n");
				break;
			}
			...
			if (rc < 0) {
				pr_debug("%s:%diommu_attach_device failed\n",
					__func__, __LINE__);
				rc = -EINVAL;
				break;
			}
			...
			pr_debug("%s:%d IOMMMU attach triggered in invalid state\n",
				__func__, __LINE__);
			rc = -EINVAL;
		}
		break;
	...
	case VIDIOC_MSM_CPP_IOMMU_DETACH: {
		if ((cpp_dev->iommu_state == CPP_IOMMU_STATE_ATTACHED) &&
			...
			rc = msm_cpp_copy_from_ioctl_ptr(&cpp_attach_info,
				ioctl_ptr);
			if (rc < 0) {
				pr_debug("CPP_IOMMU_DETTACH copy from user fail\n");
				break;
			}
			...
			if (cpp_dev->security_mode == SECURE_MODE)
				...
				rc = cam_smmu_ops(cpp_dev->iommu_hdl,
					CAM_SMMU_DETACH);
			if (rc < 0) {
				pr_debug("%s:%diommu detach failed\n", __func__,
					__LINE__);
				rc = -EINVAL;
				break;
			}
	...
	struct file *file, unsigned int cmd, void *arg)
{
	...
	if ((arg == NULL) || (file == NULL)) {
		pr_debug("Invalid input parameters arg %pK, file %pK\n",
			arg, file);
		return -EINVAL;
	}
	...
	if (sd == NULL) {
		pr_debug("Invalid input parameter sd %pK\n", sd);
		return -EINVAL;
	}
	...
	unsigned int cmd, unsigned long arg)
{
	...
	cpp_dev = v4l2_get_subdevdata(sd);
	if (!vdev || !cpp_dev) {
		pr_debug("Invalid vdev %pK or cpp_dev %pK structures!\n",
			vdev, cpp_dev);
		return -EINVAL;
	}
	mutex_lock(&cpp_dev->mutex);
	...
	if (!kp_ioctl.ioctl_ptr) {
		pr_debug("%s: Invalid ioctl pointer\n", __func__);
		mutex_unlock(&cpp_dev->mutex);
		return -EINVAL;
	}
	...
	case VIDIOC_MSM_CPP_CFG32:
	{
		...
		if (cpp_frame) {
			...
			pr_debug("%s: Error getting frame\n", __func__);
			mutex_unlock(&cpp_dev->mutex);
			rc = -EINVAL;
		}
	...
	uint32_t buff_mgr_ops, uint32_t id, void *arg)
{
	...
	default: {
		pr_debug("unsupported buffer manager ioctl\n");
		break;
	}
	}
	return rc;
...
static int cpp_probe(struct platform_device *pdev)
{
	...
	if (!cpp_dev->base) {
		rc = -ENOMEM;
		pr_debug("failed to get cpp_base\n");
	...
	if (!cpp_dev->vbif_base) {
		rc = -ENOMEM;
		pr_debug("failed to get vbif_base\n");
	...
	if (!cpp_dev->cpp_hw_base) {
		rc = -ENOMEM;
		pr_debug("failed to get cpp_hw_base\n");
	...
	if (rc < 0) {
		pr_debug("%s: failed to get the clocks\n", __func__);
		goto mem_err;
	}
	...
	if (of_find_property(pdev->dev.of_node, "qcom,cpp-cx-ipeak", NULL)) {
		...
		if (cpp_dev->cpp_cx_ipeak) {
			...
			pr_debug("Cx ipeak Registration Unsuccessful\n");
	}
	...
	if (rc < 0) {
		...
		pr_debug("%s: failed to get micro_iface_reset\n",
				__func__);
		goto get_reg_err;
	}
	rc = msm_camera_get_regulator_info(pdev, &cpp_dev->cpp_vdd,
		&cpp_dev->num_reg);
	if (rc < 0) {
		pr_debug("%s: failed to get the regulators\n", __func__);
		goto get_reset_err;
	}
