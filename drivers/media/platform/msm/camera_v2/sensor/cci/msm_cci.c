	...
	enum cci_i2c_master_t master)
{
	...
	if (rc < 0) {
		pr_debug("%s:%d wait failed\n", __func__, __LINE__);
	} else if (rc == 0) {
		pr_debug("%s:%d wait timeout\n", __func__, __LINE__);
	...
	enum cci_i2c_queue_t queue)
{
	...
	if ((read_val + len + 1) > cci_dev->cci_i2c_queue_info[
		...
		if (rc <= 0) {
			pr_debug("%s: wait_for_completion_timeout %d\n",
				 __func__, __LINE__);
			if (rc == 0)
				rc = -ETIMEDOUT;
	...
	enum cci_i2c_sync sync_en)
{
	...
	if (i2c_cmd == NULL) {
		pr_debug("%s:%d Failed line\n", __func__,
			__LINE__);
		return -EINVAL;
	}

	if ((!cmd_size) || (cmd_size > CCI_I2C_MAX_WRITE)) {
		pr_debug("%s:%d failed: invalid cmd_size %d\n",
	...
	if (i2c_msg->addr_type >= MSM_CAMERA_I2C_ADDR_TYPE_MAX) {
		pr_debug("%s:%d failed: invalid addr_type 0x%X\n",
			__func__, __LINE__, i2c_msg->addr_type);
		return -EINVAL;
	}
	if (i2c_msg->data_type >= MSM_CAMERA_I2C_DATA_TYPE_MAX) {
		pr_debug("%s:%d failed: invalid data_type 0x%X\n",
	...
	struct msm_camera_cci_ctrl *c_ctrl)
{
	...
	if (master >= MASTER_MAX || master < 0) {
		pr_debug("%s:%d Invalid I2C master %d\n",
			__func__, __LINE__, master);
		return -EINVAL;
	}
	...
	if (rc < 0) {
		pr_debug("%s:%d msm_cci_set_clk_param failed rc = %d\n",
			__func__, __LINE__, rc);
		return rc;
	}
	...
	rc = msm_cci_validate_queue(cci_dev,
		...
		master, queue);
	if (rc < 0) {
		pr_debug("%s:%d Initial validataion failed rc %d\n", __func__,
			__LINE__, rc);
		goto ERROR;
	}

	if (c_ctrl->cci_info->retries > CCI_I2C_READ_MAX_RETRIES) {
		pr_debug("%s:%d More than max retries\n", __func__,
			__LINE__);
		goto ERROR;
	}

	if (read_cfg->data == NULL) {
		pr_debug("%s:%d Data ptr is NULL\n", __func__,
			__LINE__);
		goto ERROR;
	}
	...
	if (rc <= 0) {
		...
		if (rc == 0)
			rc = -ETIMEDOUT;
		pr_debug("%s: %d wait_for_completion_timeout rc = %d\n",
			 __func__, __LINE__, rc);
		msm_cci_flush_queue(cci_dev, master);
		goto ERROR;
	} else {
	...
	if (read_words != exp_words) {
		pr_debug("%s:%d read_words = %d, exp words = %d\n", __func__,
	...
	struct msm_camera_cci_ctrl *c_ctrl)
{
	...
	if (!sd || !c_ctrl) {
		pr_debug("%s:%d sd %pK c_ctrl %pK\n", __func__,
			__LINE__, sd, c_ctrl);
		return -EINVAL;
	}
	if (!c_ctrl->cci_info) {
		pr_debug("%s:%d cci_info NULL\n", __func__, __LINE__);
		return -EINVAL;
	}
	cci_dev = v4l2_get_subdevdata(sd);
	if (!cci_dev) {
		pr_debug("%s:%d cci_dev NULL\n", __func__, __LINE__);
		return -EINVAL;
	}
	if (cci_dev->cci_state != CCI_STATE_ENABLED) {
		pr_debug("%s invalid cci state %d\n",
	...
	if (c_ctrl->cci_info->cci_i2c_master >= MASTER_MAX
		...
		pr_debug("%s:%d Invalid I2C master addr\n", __func__, __LINE__);
		return -EINVAL;
	}
	...
	enum cci_i2c_sync sync_en)
{
	...
	if (!cci_i2c_write_cfg_w->reg_setting) {
		pr_debug("%s: %d Couldn't allocate memory\n", __func__, __LINE__);
		kfree(write_async);
		return -ENOMEM;
	}
...
static int32_t msm_cci_pinctrl_init(struct cci_device *cci_dev)
{
	...
	if (IS_ERR_OR_NULL(cci_pctrl->pinctrl)) {
		pr_debug("%s:%d devm_pinctrl_get cci_pinctrl failed\n",
			__func__, __LINE__);
		return -EINVAL;
	}
	...
	if (IS_ERR_OR_NULL(cci_pctrl->gpio_state_active)) {
		pr_debug("%s:%d look up state  for active state failed\n",
			__func__, __LINE__);
		return -EINVAL;
	}
	...
	if (IS_ERR_OR_NULL(cci_pctrl->gpio_state_suspend)) {
		pr_debug("%s:%d look up state for suspend state failed\n",
			__func__, __LINE__);
		return -EINVAL;
	}
	return 0;
...
static uint32_t msm_cci_cycles_per_ms(unsigned long clk)
{
	...
	if (clk)
		...
		pr_debug("%s:%d, failed: Can use default: %d\n",
	...
	struct msm_camera_cci_ctrl *c_ctrl)
{
	...
	cci_dev = v4l2_get_subdevdata(sd);
	if (!cci_dev || !c_ctrl) {
		pr_debug("%s:%d failed: invalid params %pK %pK\n", __func__,
	...
	struct msm_camera_cci_ctrl *c_ctrl)
{
	...
	if (rc < 0) {
		pr_debug("%s:%d cci config_vreg failed\n", __func__, __LINE__);
	...
	rc = msm_camera_enable_vreg(&cci_dev->pdev->dev, cci_dev->cci_vreg,
		cci_dev->regulator_count, NULL, 0, &cci_dev->cci_reg_ptr[0], 1);
	if (rc < 0) {
		pr_debug("%s:%d cci enable_vreg failed\n", __func__, __LINE__);
	...
	for (i = 0; i < MASTER_MAX; i++) {
		if (!cci_dev->write_wq[i]) {
			pr_debug("Failed to flush write wq\n");
			rc = -ENOMEM;
			goto reset_complete_failed;
		} else {
...
static int32_t msm_cci_release(struct v4l2_subdev *sd)
{
	...
	if (!cci_dev->ref_count || cci_dev->cci_state != CCI_STATE_ENABLED) {
		pr_debug("%s invalid ref count %d / cci state %d\n",
	...
	rc = msm_camera_enable_vreg(&cci_dev->pdev->dev, cci_dev->cci_vreg,
		cci_dev->regulator_count, NULL, 0, &cci_dev->cci_reg_ptr[0], 0);
	if (rc < 0)
		pr_debug("%s:%d cci disable_vreg failed\n", __func__, __LINE__);

	rc = msm_camera_config_vreg(&cci_dev->pdev->dev, cci_dev->cci_vreg,
		cci_dev->regulator_count, NULL, 0, &cci_dev->cci_reg_ptr[0], 0);
	if (rc < 0)
		pr_debug("%s:%d cci unconfig_vreg failed\n", __func__, __LINE__);
...
static int msm_cci_probe(struct platform_device *pdev)
{
	...
	rc = msm_camera_get_clk_info_and_rates(pdev,
		...
		&new_cci_dev->num_clk);
	if (rc < 0) {
		pr_debug("%s: msm_cci_get_clk_info() failed\n", __func__);
		kfree(new_cci_dev);
		return -EFAULT;
	}
	...
	if (rc < 0) {
		pr_debug("%s: msm_camera_get_dt_vreg_data fail\n", __func__);
		rc = -EFAULT;
		goto cci_release_mem;
	}
	...
	if (rc)
		pr_debug("%s: failed to add child nodes, rc=%d\n", __func__, rc);
	...
	for (i = 0; i < MASTER_MAX; i++) {
		...
		if (!new_cci_dev->write_wq[i])
			pr_debug("Failed to create write wq\n");
	}
