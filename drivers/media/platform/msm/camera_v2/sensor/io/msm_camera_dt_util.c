	...
	uint16_t power_setting_size)
{
	...
	if (!cam_vreg || !power_setting) {
		pr_debug("%s:%d failed: cam_vreg %pK power_setting %pK\n",
	...
	for (i = 0; i < power_setting_size; i++) {
		...
		default:
			pr_debug("%s:%d invalid seq_val %d\n", __func__,
				    ...
				    struct  msm_sensor_info_t **s_info)
{
	...
	if (of_get_property(of_node, "qcom,csiphy-sd-index", &count)) {
		...
		if (count > 2) {
			pr_debug("%s qcom,csiphy-sd-index count %d > 2\n",
				__func__, count);
			goto ERROR;
		}
		...
		pr_debug("%s:%d qcom,csiphy-sd-index not present\n", __func__,
			__LINE__);
		rc = -EINVAL;
		goto ERROR;
	}
	...
	if (of_get_property(of_node, "qcom,csid-sd-index", &count)) {
		count /= sizeof(uint32_t);
		if (count > 2) {
			pr_debug("%s qcom,csid-sd-index count %d > 2\n",
				__func__, count);
			rc = -EINVAL;
			goto ERROR;
		}
		...
		pr_debug("%s:%d qcom,csid-sd-index not present\n", __func__,
			__LINE__);
		rc = -EINVAL;
		goto ERROR;
	}
	...
	uint16_t gpio_array_size)
{
	...
	if (rc != -EINVAL) {
		if (rc < 0) {
			pr_debug("%s:%d read qcom,gpio-ir-p failed rc %d\n",
				__func__, __LINE__, rc);
			goto ERROR;
		} else if (val >= gpio_array_size) {
			pr_debug("%s:%d qcom,gpio-ir-p invalid %d\n",
	...
	if (rc != -EINVAL) {
		if (rc < 0) {
			pr_debug("%s:%d read qcom,gpio-ir-m failed rc %d\n",
				__func__, __LINE__, rc);
			goto ERROR;
		} else if (val >= gpio_array_size) {
			pr_debug("%s:%d qcom,gpio-ir-m invalid %d\n",
				__func__, __LINE__, val);
			rc = -EINVAL;
			goto ERROR;
		}
	...
	if (rc != -EINVAL) {
		if (rc < 0) {
			pr_debug("%s:%d read qcom,gpio-vana failed rc %d\n",
				__func__, __LINE__, rc);
			goto ERROR;
		} else if (val >= gpio_array_size) {
			pr_debug("%s:%d qcom,gpio-vana invalid %d\n",
				__func__, __LINE__, val);
			rc = -EINVAL;
			goto ERROR;
		}
	...
	if (rc != -EINVAL) {
		if (rc < 0) {
			pr_debug("%s:%d read qcom,gpio-vio failed rc %d\n",
				__func__, __LINE__, rc);
			goto ERROR;
		} else if (val >= gpio_array_size) {
			pr_debug("%s:%d qcom,gpio-vio invalid %d\n",
				__func__, __LINE__, val);
			goto ERROR;
		}
	...
	if (rc != -EINVAL) {
		if (rc < 0) {
			pr_debug("%s:%d read qcom,gpio-vaf failed rc %d\n",
				__func__, __LINE__, rc);
			goto ERROR;
		} else if (val >= gpio_array_size) {
			pr_debug("%s:%d qcom,gpio-vaf invalid %d\n",
				__func__, __LINE__, val);
			rc = -EINVAL;
			goto ERROR;
		}
	...
	if (rc != -EINVAL) {
		if (rc < 0) {
			pr_debug("%s:%d read qcom,gpio-vdig failed rc %d\n",
				__func__, __LINE__, rc);
			goto ERROR;
		} else if (val >= gpio_array_size) {
			pr_debug("%s:%d qcom,gpio-vdig invalid %d\n",
				__func__, __LINE__, val);
			rc = -EINVAL;
			goto ERROR;
		}
	...
	if (rc != -EINVAL) {
		if (rc < 0) {
			pr_debug("%s:%d read qcom,gpio-reset failed rc %d\n",
				__func__, __LINE__, rc);
			goto ERROR;
		} else if (val >= gpio_array_size) {
			pr_debug("%s:%d qcom,gpio-reset invalid %d\n",
				__func__, __LINE__, val);
			rc = -EINVAL;
			goto ERROR;
		}
	...
	if (rc != -EINVAL) {
		if (rc < 0) {
			pr_debug("%s:%d read qcom,gpio-standby failed rc %d\n",
				__func__, __LINE__, rc);
			goto ERROR;
		} else if (val >= gpio_array_size) {
			pr_debug("%s:%d qcom,gpio-standby invalid %d\n",
				__func__, __LINE__, val);
			rc = -EINVAL;
			goto ERROR;
		}
	...
	if (rc != -EINVAL) {
		if (rc < 0) {
			pr_debug("%s:%d read qcom,gpio-af-pwdm failed rc %d\n",
				__func__, __LINE__, rc);
			goto ERROR;
		} else if (val >= gpio_array_size) {
			pr_debug("%s:%d qcom,gpio-af-pwdm invalid %d\n",
				__func__, __LINE__, val);
			rc = -EINVAL;
			goto ERROR;
		}
	...
	if (rc != -EINVAL) {
		if (rc < 0) {
			pr_debug("%s:%d read qcom,gpio-flash-en failed rc %d\n",
				__func__, __LINE__, rc);
			goto ERROR;
		} else if (val >= gpio_array_size) {
			pr_debug("%s:%d qcom,gpio-flash-en invalid %d\n",
				__func__, __LINE__, val);
			rc = -EINVAL;
			goto ERROR;
		}
	...
	if (rc != -EINVAL) {
		if (rc < 0) {
			pr_debug("%s:%d read qcom,gpio-flash-now failed rc %d\n",
				__func__, __LINE__, rc);
			goto ERROR;
		} else if (val >= gpio_array_size) {
			pr_debug("%s:%d qcom,gpio-flash-now invalid %d\n",
				__func__, __LINE__, val);
			rc = -EINVAL;
			goto ERROR;
		}
	...
	if (rc != -EINVAL) {
		if (rc < 0) {
			pr_debug("%s:%dread qcom,gpio-flash-reset failed rc %d\n",
				__func__, __LINE__, rc);
			goto ERROR;
		} else if (val >= gpio_array_size) {
			pr_debug("%s:%d qcom,gpio-flash-reset invalid %d\n",
				__func__, __LINE__, val);
			rc = -EINVAL;
			goto ERROR;
		}
	...
	if (rc != -EINVAL) {
		if (rc < 0) {
			pr_debug("%s:%d read qcom,gpio-custom1 failed rc %d\n",
				__func__, __LINE__, rc);
			goto ERROR;
		} else if (val >= gpio_array_size) {
			pr_debug("%s:%d qcom,gpio-custom1 invalid %d\n",
				__func__, __LINE__, val);
			rc = -EINVAL;
			goto ERROR;
		}
	...
	if (rc != -EINVAL) {
		if (rc < 0) {
			pr_debug("%s:%d read qcom,gpio-custom2 failed rc %d\n",
				__func__, __LINE__, rc);
			goto ERROR;
		} else if (val >= gpio_array_size) {
			pr_debug("%s:%d qcom,gpio-custom2 invalid %d\n",
				__func__, __LINE__, val);
			rc = -EINVAL;
			goto ERROR;
		}
	...
	if (rc != -EINVAL) {
		if (rc < 0) {
			pr_debug("%s:%d read qcom,gpio-custom3 failed rc %d\n",
				__func__, __LINE__, rc);
			goto ERROR;
		} else if (val >= gpio_array_size) {
			pr_debug("%s:%d qcom,gpio-custom3 invalid %d\n",
				__func__, __LINE__, val);
			rc = -EINVAL;
			goto ERROR;
		}
	...
	struct msm_pinctrl_info *sensor_pctrl, struct device *dev)
{
	...
	if (IS_ERR_OR_NULL(sensor_pctrl->pinctrl)) {
		pr_debug("%s:%d Getting pinctrl handle failed\n",
			__func__, __LINE__);
		return -EINVAL;
	}
	...
	if (IS_ERR_OR_NULL(sensor_pctrl->gpio_state_active)) {
		pr_debug("%s:%d Failed to get the active state pinctrl handle\n",
			__func__, __LINE__);
		return -EINVAL;
	}
	...
	if (IS_ERR_OR_NULL(sensor_pctrl->gpio_state_suspend)) {
		pr_debug("%s:%d Failed to get the suspend state pinctrl handle\n",
				__func__, __LINE__);
		return -EINVAL;
	}
	return 0;
	...
	struct msm_camera_gpio_conf *gconf, int val)
{
	...
	if (!gconf) {
		pr_debug("ERR:%s: Input Parameters are not proper\n", __func__);
		return -EINVAL;
	}
	...
	default:
		pr_debug("%s:%d Invalid VREG seq val %d\n", __func__,
			__LINE__, seq_val);
		return -EINVAL;
	}
	...
	struct device_node *of_node)
{
	...
	/* Validate input parameters */
	if (!of_node) {
		pr_debug("failed: invalid param of_node %pK\n", of_node);
		return -EINVAL;
	}
	...
	*gpio_conf = gconf;
	...
	rc = msm_camera_get_dt_gpio_req_tbl(of_node, gconf, gpio_array,
		gpio_array_size);
	if (rc < 0) {
		pr_debug("failed in msm_camera_get_dt_gpio_req_tbl\n");
		goto FREE_GPIO_CONF;
	}
	...
	rc = msm_camera_init_gpio_pin_tbl(of_node, gconf, gpio_array,
		gpio_array_size);
	if (rc < 0) {
		pr_debug("failed in msm_camera_init_gpio_pin_tbl\n");
	...
	struct msm_camera_i2c_client *sensor_i2c_client)
{
	...
	if (!ctrl || !sensor_i2c_client) {
		pr_debug("failed ctrl %pK sensor_i2c_client %pK\n", ctrl,
			sensor_i2c_client);
		return -EINVAL;
	}
	...
	if (ret < 0) {
		pr_debug("%s:%d Initialization of pinctrl failed\n",
				__func__, __LINE__);
	...
	if (ctrl->cam_pinctrl_status) {
		...
		if (ret)
			pr_debug("%s:%d cannot set pin to active state\n",
				__func__, __LINE__);
	}
	for (index = 0; index < ctrl->power_setting_size; index++) {
		...
		case SENSOR_VREG:
			...
			if (power_setting->seq_val < ctrl->num_vreg)
				...
				pr_debug("%s: %d usr_idx:%d dts_idx:%d\n",
					__func__, __LINE__,
			...
			rc = msm_cam_sensor_handle_reg_gpio(
				...
				ctrl->gpio_conf, 1);
			if (rc < 0) {
				pr_debug("ERR:%s Error in handling VREG GPIO\n",
					__func__);
				goto power_up_failed;
			}
			break;
...
power_up_failed:
	...
	for (index--; index >= 0; index--) {
		...
		case SENSOR_VREG:
			if (power_setting->seq_val < ctrl->num_vreg)
				...
				pr_debug("%s:%d:seq_val: %d > num_vreg: %d\n",
					__func__, __LINE__,
					power_setting->seq_val, ctrl->num_vreg);
	...
	if (ctrl->cam_pinctrl_status) {
		...
		if (ret)
			pr_debug("%s:%d cannot set pin to suspend state\n",
				__func__, __LINE__);
	...
	for (index = 0; index < ctrl->power_down_setting_size; index++) {
		...
		case SENSOR_VREG:
			...
			if (ps) {
				...
				pr_debug("%s error in power up/down seq data\n",
								__func__);
			ret = msm_cam_sensor_handle_reg_gpio(pd->seq_val,
				ctrl->gpio_conf, GPIOF_OUT_INIT_LOW);
			if (ret < 0)
				pr_debug("ERR:%s Error while disabling VREG GPIO\n",
					__func__);
			break;
		case SENSOR_I2C_MUX:
			if (ctrl->i2c_conf && ctrl->i2c_conf->use_i2c_mux)
