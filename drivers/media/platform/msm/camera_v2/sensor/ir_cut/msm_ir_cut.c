	...
	struct msm_ir_cut_ctrl_t *ir_cut_ctrl)
{
	...
	if (ir_cut_ctrl->ir_cut_state == MSM_CAMERA_IR_CUT_RELEASE) {
		pr_debug("%s:%d Invalid ir_cut state = %d\n",
	...
	if (rc < 0) {
		pr_debug("%s:%d camera_ir_cut_on failed rc = %d\n",
	...
	struct msm_ir_cut_cfg_data_t *ir_cut_data)
{
	...
	if (ir_cut_ctrl->gconf) {
		...
		if (rc < 0) {
			pr_debug("ERR:%s:Failed in selecting state: %d\n",
				__func__, rc);

			return rc;
		}
	} else {
		pr_debug("%s: No IR CUT GPIOs\n", __func__);
		return 0;
	}
	...
	struct msm_ir_cut_cfg_data_t *ir_cut_data)
{
	...
	if (rc < 0) {
		pr_debug("%s:%d failed invalid ir_cut_driver_type %d\n",
	...
	if (rc < 0) {
		pr_debug("%s:%d camera_ir_cut_init failed rc = %d\n",
			__func__, __LINE__, rc);
		return rc;
	}
	...
	struct msm_ir_cut_ctrl_t *fctrl)
{
	...
	if (rc < 0) {
		pr_debug("%s:%d msm_ir_cut_get_gpio_dt_data failed rc %d\n",
			__func__, __LINE__, rc);
		return rc;
	}
...
static int32_t msm_ir_cut_platform_probe(struct platform_device *pdev)
{
	...
	if (rc < 0) {
		pr_debug("%s:%d msm_ir_cut_get_dt_data failed\n",
			__func__, __LINE__);
		kfree(ir_cut_ctrl);
		return -EINVAL;
	}
	...
	if (rc < 0) {
		pr_debug("ERR:%s: Error in reading IR CUT pinctrl\n",
			__func__);
...
static int __init msm_ir_cut_init_module(void)
{
	...
	pr_debug("platform probe for ir_cut failed\n");
