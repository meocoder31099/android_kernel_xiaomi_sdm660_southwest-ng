...
static void v4l_print_format(const void *arg, bool write_only)
{
	...
	case V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE:
		...
		for (i = 0; i < planes; i++)
			no_printk(KERN_DEBUG "plane %u: bytesperline=%u sizeimage=%u\n", i,
	...
	case V4L2_BUF_TYPE_SLICED_VBI_OUTPUT:
		...
		for (i = 0; i < 24; i++)
			no_printk(KERN_DEBUG "line[%02u]=0x%04x, 0x%04x\n", i,
...
static void v4l_print_buffer(const void *arg, bool write_only)
{
	...
	no_printk(KERN_DEBUG "timecode=%02d:%02d:%02d type=%d, flags=0x%08x, frames=%d, userbits=0x%08x\n",
...
static void v4l_print_event(const void *arg, bool write_only)
{
	...
	case V4L2_EVENT_VSYNC:
		no_printk(KERN_DEBUG "field=%s\n",
			prt_names(p->u.vsync.field, v4l2_field_names));
		break;
	case V4L2_EVENT_CTRL:
		c = &p->u.ctrl;
		no_printk(KERN_DEBUG "changes=0x%x, type=%u, ",
			c->changes, c->type);
