...
static void sdio_enable_vendor_specific_settings(struct mmc_card *card)
{
	...
	if (mmc_enable_qca6574_settings(card) ||
		...
		if (ret) {
			pr_debug("%s: failed to write to fn 0xf2 %d\n",
		...
		if (ret) {
			pr_debug("%s: failed to read fn 0xf1 %d\n",
			mmc_hostname(card->host), ret);
			goto out;
		}
		...
		if (ret) {
			pr_debug("%s: failed to write to fn 0xf1 %d\n",
				mmc_hostname(card->host), ret);
			goto out;
		}
		...
		if (ret) {
			pr_debug("%s: failed to read fn 0xf0 %d\n",
			mmc_hostname(card->host), ret);
			goto out;
		}
		...
		if (ret) {
			pr_debug("%s: failed to write to fn 0xf0 %d\n",
				mmc_hostname(card->host), ret);
			goto out;
		}
	}
