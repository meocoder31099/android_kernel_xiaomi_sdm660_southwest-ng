...
ppp_async_init(void)
{
	...
	if (err != 0)
		no_printk(KERN_ERR "PPP_async: error %d registering line disc.\n",
		       err);
	return err;
}
		...
		char *flags, int count)
{
 ...
 nomem:
	no_printk(KERN_ERR "PPPasync: no memory (input pkt)\n");
	ap->state |= SC_TOSS;
}
...
static void __exit ppp_async_cleanup(void)
{
	if (tty_unregister_ldisc(N_PPP) != 0)
		no_printk(KERN_ERR "failed to unregister PPP line discipline\n");
}
