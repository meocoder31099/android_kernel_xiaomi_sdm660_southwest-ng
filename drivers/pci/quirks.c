...
static int __init pci_apply_final_quirks(void)
{
	...
	if (pci_cache_line_size)
		no_printk(KERN_DEBUG "PCI: CLS %u bytes\n",
	...
	for_each_pci_dev(dev) {
		...
		if (!pci_cache_line_size) {
			...
			no_printk(KERN_DEBUG "PCI: CLS mismatch (%u != %u), using %u bytes\n",
			       cls << 2, tmp << 2,
	...
	if (!pci_cache_line_size) {
		no_printk(KERN_DEBUG "PCI: CLS %u bytes, default %u\n",
...
static void nvbridge_check_legacy_irq_routing(struct pci_dev *dev)
{
	...
	if (cfg & ((1 << 2) | (1 << 15))) {
		no_printk(KERN_INFO "Rewriting IRQ routing register on MCP55\n");
		cfg &= ~((1 << 2) | (1 << 15));
