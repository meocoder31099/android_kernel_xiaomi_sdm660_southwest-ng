...
static int msm_ext_disp_extcon_register(struct msm_ext_disp *ext_disp, int id)
{
	...
	if (ret) {
		pr_debug("audio registration failed\n");
		return ret;
	}
		...
		enum msm_ext_disp_cable_state new_state)
{
	...
	if (!ext_disp->ops) {
		pr_debug("codec not registered, skip notification\n");
		ret = -EPERM;
		goto end;
	}
	...
	if (ret)
		pr_debug("Failed to set state. Error = %d\n", ret);
	else
		...
		enum msm_ext_disp_cable_state state)
{
	...
	if (state == EXT_DISPLAY_CABLE_CONNECT) {
		...
		if (ret)
			pr_debug("error setting audio codec\n");
	} else {
		...
		struct msm_ext_disp_audio_codec_ops *ops)
{
	...
	if (ext_disp->ops) {
		pr_debug("Codec already registered\n");
		ret = -EINVAL;
		goto end;
	}
		...
		struct msm_ext_disp_codec_id *codec)
{
	...
	if (!ext_disp->ops) {
		pr_debug("Codec is not registered\n");
		ret = -EINVAL;
		goto end;
	}
...
static int msm_ext_disp_validate_intf(struct msm_ext_disp_init_data *init_data)
{
	...
	if (!init_data) {
		pr_debug("Invalid init_data\n");
		return -EINVAL;
	}

	if (!init_data->pdev) {
		pr_debug("Invalid display intf pdev\n");
		return -EINVAL;
	}
	...
	if (init_data->codec.type >= EXT_DISPLAY_TYPE_MAX ||
		...
		pr_debug("Invalid codec info type(%d), ctrl(%d) stream(%d)\n",
	...
	if (!ops->audio_info_setup   ||
	    ...
	    !ops->acknowledge        ||
	    !ops->ready) {
		pr_debug("Invalid codec operation pointers\n");
		return -EINVAL;
	}
		...
		struct msm_ext_disp_init_data *init_data)
{
	...
	if (!ret) {
		pr_debug("%s already registered. ctrl(%d) stream(%d)\n",
...
static int msm_ext_disp_probe(struct platform_device *pdev)
{
	...
	if (!pdev) {
		pr_debug("No platform device found\n");
		ret = -ENODEV;
		goto end;
	}
	...
	if (ret) {
		pr_debug("Failed to add child devices. Error = %d\n", ret);
		goto child_node_failure;
	} else {
