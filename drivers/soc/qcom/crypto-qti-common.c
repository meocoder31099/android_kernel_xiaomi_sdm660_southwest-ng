...
static int ice_check_fuse_setting(struct crypto_vops_qti_entry *ice_entry)
{
	...
	if (regval) {
		pr_debug("%s: error: ICE_ERROR_HW_DISABLE_FUSE_BLOWN\n",
				__func__);
		return -EPERM;
	}
	return 0;
...
static int ice_check_version(struct crypto_vops_qti_entry *ice_entry)
{
	...
	if (major < ICE_CORE_CURRENT_MAJOR_VERSION) {
		pr_debug("%s: Unknown ICE device at %lu, rev %d.%d.%d\n",
			   ...
			   void **priv_data)
{
	...
	if (err) {
		pr_debug("%s: check version failed, err %d\n", __func__, err);
		return err;
	}
...
static int ice_wait_bist_status(struct crypto_vops_qti_entry *ice_entry)
{
	...
	if (regval) {
		pr_debug("%s: wait bist status failed, reg %d\n",
				__func__, regval);
		return -ETIMEDOUT;
	}
...
int crypto_qti_enable(void *priv_data)
{
	...
	if (!ice_entry) {
		pr_debug("%s: vops ice data is invalid\n", __func__);
		return -EINVAL;
	}
			       ...
			       u8 data_unit_mask, int capid)
{
	...
	if (err1) {
		pr_debug("%s: program key failed with error %d\n",
			__func__, err1);
		err2 = crypto_qti_invalidate_key(ice_entry, slot);
		if (err2) {
			pr_debug("%s: invalidate key failed with error %d\n",
				__func__, err2);
		}
	}
