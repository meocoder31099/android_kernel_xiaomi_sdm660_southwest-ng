...
static void dump_cpu_alive_mask(struct msm_watchdog_data *wdog_dd)
{
	...
	scnprintf(alive_mask_buf, MASK_SIZE, "%*pb1", cpumask_pr_args(
				&wdog_dd->alive_mask));
	dev_dbg(wdog_dd->dev, "cpu alive mask from last pet %s\n",
				alive_mask_buf);
}
				...
				const char *buf, size_t count)
{
	...
	if (ret) {
		dev_dbg(wdog_dd->dev, "invalid user input\n");
		return ret;
	}
	if (disable == 1) {
		mutex_lock(&wdog_dd->disable_lock);
		...
		pr_debug("invalid operation, only disable = 1 supported\n");
		return -EINVAL;
	}
	return count;
}
...
static int msm_watchdog_remove(struct platform_device *pdev)
{
	...
	dev_dbg(wdog_dd->dev, "MSM Watchdog Exit - Deactivated\n");
	del_timer_sync(&wdog_dd->pet_timer);
...
void msm_trigger_wdog_bite(void)
{
	...
	mdelay(10000);
	pr_debug("Wdog - STS: 0x%x, CTL: 0x%x, BARK TIME: 0x%x, BITE TIME: 0x%x",
...
static irqreturn_t wdog_bark_handler(int irq, void *dev_id)
{
	...
	dev_dbg(wdog_dd->dev, "Watchdog bark! Now = %lu.%06lu\n",
			(unsigned long) t, nanosec_rem / 1000);

	nanosec_rem = do_div(wdog_dd->last_pet, 1000000000);
	dev_dbg(wdog_dd->dev, "Watchdog last pet at %lu.%06lu\n",
...
static int init_watchdog_sysfs(struct msm_watchdog_data *wdog_dd)
{
	...
	if (error)
		dev_dbg(wdog_dd->dev, "cannot create sysfs attribute\n");
...
static void minidump_reg_init_log_buf(void)
{
	...
	if (msm_minidump_add_region(&md_entry) < 0)
		pr_debug("Failed to add init_log_buf in Minidump\n");
}
...
static void log_buf_work_fn(struct work_struct *work)
{
	...
	if (!addr) {
		dev_dbg(wdog_data->dev, "log_buf symbol not found\n");
		goto out;
	}

	log_buf_size = (unsigned int *)kallsyms_lookup_name("log_buf_len");
	if (!log_buf_size) {
		dev_dbg(wdog_data->dev, "log_buf_len symbol not found\n");
		goto out;
	}
	...
	if (!init_log_buf) {
		dev_dbg(wdog_data->dev, "log_buf dma_alloc_coherent failed\n");
		goto out;
	}
...
static void init_watchdog_data(struct msm_watchdog_data *wdog_dd)
{
	...
	if (wdog_dd->irq_ppi) {
		...
		if (!wdog_dd->wdog_cpu_dd) {
			dev_dbg(wdog_dd->dev, "fail to allocate cpu data\n");
			return;
		}
		*raw_cpu_ptr(wdog_dd->wdog_cpu_dd) = wdog_dd;
		ret = request_percpu_irq(wdog_dd->bark_irq, wdog_ppi_bark,
					...
					wdog_dd->wdog_cpu_dd);
		if (ret) {
			dev_dbg(wdog_dd->dev, "failed to request bark irq\n");
			free_percpu(wdog_dd->wdog_cpu_dd);
			return;
		}
	} else {
	...
	dev_dbg(wdog_dd->dev, "MSM Watchdog Initialized\n");
}
					...
					struct msm_watchdog_data *pdata)
{
	...
	if (unlikely(!(devm_request_mem_region(&pdev->dev, pdata->phys_base,
		...
		dev_dbg(&pdev->dev, "%s cannot reserve watchdog region\n",
								__func__);
		return -ENXIO;
	}
	pdata->base  = devm_ioremap(&pdev->dev, pdata->phys_base,
							pdata->size);
	if (!pdata->base) {
		dev_dbg(&pdev->dev, "%s cannot map wdog register space\n",
				__func__);
		return -ENXIO;
	}
	...
	if (res) {
		...
		dev_dbg(&pdev->dev, "wdog absent resource not present\n");
	}
	...
	if (ret) {
		dev_dbg(&pdev->dev, "reading bark time failed\n");
		return -ENXIO;
	}
	ret = of_property_read_u32(node, "qcom,pet-time", &pdata->pet_time);
	if (ret) {
		dev_dbg(&pdev->dev, "reading pet time failed\n");
		return -ENXIO;
	}
	pdata->do_ipi_ping = of_property_read_bool(node, "qcom,ipi-ping");
	if (!pdata->bark_time) {
		dev_dbg(&pdev->dev, "%s watchdog bark time not setup\n",
								__func__);
		return -ENXIO;
	}
	if (!pdata->pet_time) {
		dev_dbg(&pdev->dev, "%s watchdog pet time not setup\n",
								__func__);
		return -ENXIO;
	}
