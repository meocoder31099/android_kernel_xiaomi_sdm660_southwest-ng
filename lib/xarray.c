...
void xa_dump_node(const struct xa_node *node)
{
	...
	pr_debug("node %px %s %d parent %px shift %d count %d values %d "
...
void xa_dump_entry(const void *entry, unsigned long index, unsigned long shift)
{
	...
	if (xa_is_node(entry)) {
		...
		pr_debug("sibling (slot %ld)\n", xa_to_sibling(entry));
	else if (xa_is_zero(entry))
		pr_debug("zero (%ld)\n", xa_to_internal(entry));
	else
		pr_debug("UNKNOWN ENTRY (%px)\n", entry);
}
