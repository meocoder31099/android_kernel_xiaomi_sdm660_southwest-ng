		...
		unsigned long bad_flags)
{
	...
	pr_debug("BUG: Bad page state in process %s  pfn:%05lx\n",
		current->comm, page_to_pfn(page));
	__dump_page(page, reason);
	bad_flags &= page->flags;
	if (bad_flags)
		pr_debug("bad because of flags: %#lx(%pGp)\n",
						bad_flags, &bad_flags);
...
static inline void show_node(struct zone *zone)
{
	if (IS_ENABLED(CONFIG_NUMA))
		no_printk("Node %d ", zone_to_nid(zone));
}
...
void show_free_areas(unsigned int filter, nodemask_t *nodemask)
{
	...
	no_printk("active_anon:%lu inactive_anon:%lu isolated_anon:%lu\n"
	...
	for_each_populated_zone(zone) {
		...
		no_printk("lowmem_reserve[]:");
		for (i = 0; i < MAX_NR_ZONES; i++)
			no_printk(KERN_CONT " %ld", zone->lowmem_reserve[i]);
		no_printk(KERN_CONT "\n");
	}
	...
	for_each_populated_zone(zone) {
		...
		if (show_mem_node_skip(filter, zone_to_nid(zone), nodemask))
			continue;
		show_node(zone);
		no_printk(KERN_CONT "%s: ", zone->name);
		...
		spin_unlock_irqrestore(&zone->lock, flags);
		for (order = 0; order < MAX_ORDER; order++) {
			no_printk(KERN_CONT "%lu*%lukB ",
			       nr[order], K(1UL) << order);
			if (nr[order])
				show_migration_types(types[order]);
		}
		no_printk(KERN_CONT "= %lukB\n", K(total));
	}

	hugetlb_show_meminfo();

	no_printk("%ld total pagecache pages\n", global_node_page_state(NR_FILE_PAGES));
...
static void __build_all_zonelists(void *data)
{
	...
	local_irq_save(flags);
	/*
	 * Explicitly disable this CPU's synchronous no_printk() before taking
	 * seqlock to prevent any no_printk() from trying to hold port->lock, for
...
static __meminit void zone_pcp_init(struct zone *zone)
{
	...
	if (populated_zone(zone))
		no_printk(KERN_DEBUG "  %s zone: %lu pages, LIFO batch:%u\n",
						...
						unsigned long *zholes_size)
{
	...
	no_printk(KERN_DEBUG "On node %d totalpages: %lu\n", pgdat->node_id,
							realtotalpages);
}
...
static void __init free_area_init_core(struct pglist_data *pgdat)
{
	...
	for (j = 0; j < MAX_NR_ZONES; j++) {
		...
		if (j == 0 && freesize > dma_reserve) {
			...
			no_printk(KERN_DEBUG "  %s zone: %lu pages reserved\n",
