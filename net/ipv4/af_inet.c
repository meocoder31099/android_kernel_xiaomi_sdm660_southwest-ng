...
static int __init ipv4_offload_init(void)
{
	...
	if (udpv4_offload_init() < 0)
		pr_debug("%s: Cannot add UDP protocol offload\n", __func__);
	if (tcpv4_offload_init() < 0)
		pr_debug("%s: Cannot add TCP protocol offload\n", __func__);
	if (ipip_offload_init() < 0)
		pr_debug("%s: Cannot add IPIP protocol offload\n", __func__);
...
static int __init inet_init(void)
{
	...
	if (inet_add_protocol(&icmp_protocol, IPPROTO_ICMP) < 0)
		pr_debug("%s: Cannot add ICMP protocol\n", __func__);
	if (inet_add_protocol(&udp_protocol, IPPROTO_UDP) < 0)
		pr_debug("%s: Cannot add UDP protocol\n", __func__);
	if (inet_add_protocol(&tcp_protocol, IPPROTO_TCP) < 0)
		pr_debug("%s: Cannot add TCP protocol\n", __func__);
#ifdef CONFIG_IP_MULTICAST
	if (inet_add_protocol(&igmp_protocol, IPPROTO_IGMP) < 0)
		pr_debug("%s: Cannot add IGMP protocol\n", __func__);
#endif
	...
	if (ip_mr_init())
		pr_debug("%s: Cannot init ipv4 mroute\n", __func__);
#endif

	if (init_inet_pernet_ops())
		pr_debug("%s: Cannot init ipv4 inet pernet ops\n", __func__);
