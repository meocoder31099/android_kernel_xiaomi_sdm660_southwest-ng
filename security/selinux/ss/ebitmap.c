...
int ebitmap_read(struct ebitmap *e, void *fp)
{
	...
	if (mapunit != BITS_PER_U64) {
		pr_debug("SELinux: ebitmap: map size %u does not "
	...
	for (i = 0; i < count; i++) {
		...
		if (rc < 0) {
			pr_debug("SELinux: ebitmap: truncated map\n");
			goto bad;
		}
		startbit = le32_to_cpu(startbit);

		if (startbit & (mapunit - 1)) {
			pr_debug("SELinux: ebitmap start bit (%d) is "
		...
		if (!n || startbit >= n->startbit + EBITMAP_SIZE) {
			...
			if (!tmp) {
				pr_debug("SELinux: ebitmap: out of memory\n");
				rc = -ENOMEM;
				goto bad;
			}
			/* round down */
			...
			pr_debug("SELinux: ebitmap: start bit %d"
