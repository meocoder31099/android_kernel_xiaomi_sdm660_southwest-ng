			...
			struct snd_ctl_elem_info *uinfo)
{
	...
	if (!codec_data) {
		dev_dbg(component->dev, "%s: codec_data is NULL\n", __func__);
		return -EINVAL;
	}
...
static int msm_ext_disp_edid_get(struct snd_kcontrol *kcontrol,
	...
	rc = msm_ext_disp_select_audio_codec(codec_data->ext_disp_core_pdev,
						 &codec_info);
	if (!codec_data->ext_disp_ops.get_audio_edid_blk || rc) {
		dev_dbg(component->dev, "%s: codec_data or get_audio_edid_blk() is NULL\n",
			__func__);
		mutex_unlock(&codec_data->dp_ops_lock);
		return -EINVAL;
	}
				       ...
				       struct snd_ctl_elem_value *ucontrol)
{
	...
	if (!codec_data->ext_disp_ops.cable_status ||
		...
		dev_dbg(component->dev, "%s: cable_status() or get_intf_id is NULL\n",
			__func__);
		rc = -EINVAL;
		goto cable_err;
	}
	...
	if (cable_state < 0) {
		dev_dbg(component->dev, "%s: Error retrieving cable state from ext_disp, err:%d\n",
	...
	if (cable_state == EXT_DISPLAY_CABLE_DISCONNECT) {
		dev_dbg(component->dev, "%s: Display cable disconnected\n",
			__func__);
	...
	if (disp_type >= 0) {
		...
		default:
			rc = -EINVAL;
			dev_dbg(component->dev, "%s: Invalid disp_type:%d\n",
			       __func__, disp_type);
			goto done;
		}
		...
		dev_dbg(component->dev, "%s: Error retrieving disp_type from ext_disp, err:%d\n",
			__func__, disp_type);
		rc = disp_type;
	}
	return rc;
				      ...
				      struct snd_ctl_elem_value *ucontrol)
{
	...
	mutex_unlock(&codec_data->dp_ops_lock);
	if (rc < 0) {
		dev_dbg(component->dev, "%s: error from acknowledge(), err:%d\n",
			__func__, rc);
	}
	return rc;
		...
		struct snd_soc_dai *dai)
{
	...
	if (!codec_data) {
		dev_dbg(dai->dev, "%s() codec_data is null\n",
			__func__);
		return -EINVAL;
	}
	...
	if (!codec_data->ext_disp_ops.cable_status || rc) {
		dev_dbg(dai->dev, "%s() cable_status is null\n",
			__func__);
		mutex_unlock(&codec_data->dp_ops_lock);
		return -EINVAL;
	}
		...
		struct snd_soc_dai *dai)
{
	...
	if (!codec_data->ext_disp_ops.audio_info_setup || rc) {
		dev_dbg(dai->dev, "%s: audio_info_setup is null\n",
			__func__);
		mutex_unlock(&codec_data->dp_ops_lock);
		return -EINVAL;
	}
		...
		struct snd_soc_dai *dai)
{
	...
	if (!codec_data->ext_disp_ops.teardown_done ||
		...
		dev_dbg(dai->dev, "%s: teardown_done or cable_status is null\n",
			__func__);
		mutex_unlock(&codec_data->dp_ops_lock);
		return;
	}
		...
		struct snd_soc_component *component)
{
	...
	if (!codec_data) {
		dev_dbg(component->dev, "%s(): fail to allocate dai data\n",
				__func__);
		return -ENOMEM;
	}
	...
	if (!of_node_parent) {
		dev_dbg(component->dev, "%s(): Parent device tree node not found\n",
				__func__);
		kfree(codec_data);
		return -ENODEV;
	}
	...
	if (!codec_data->ext_disp_core_pdev) {
		dev_dbg(component->dev, "%s(): can't get parent pdev\n",
			__func__);
		kfree(codec_data);
		return -ENODEV;
	}
	...
	if (msm_ext_disp_register_audio_codec(codec_data->ext_disp_core_pdev,
		...
		dev_dbg(component->dev, "%s(): can't register with ext disp core",
				__func__);
		kfree(codec_data);
		return -ENODEV;
	}
