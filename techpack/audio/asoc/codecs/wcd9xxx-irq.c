...
static irqreturn_t wcd9xxx_irq_thread(int irq, void *data)
{
	...
	if (unlikely(wcd9xxx_lock_sleep(wcd9xxx_res) == false)) {
		dev_dbg(wcd9xxx_res->dev, "Failed to hold suspend\n");
		return IRQ_NONE;
	}
...
int wcd9xxx_irq_init(struct wcd9xxx_core_resource *wcd9xxx_res)
{
	...
	if (!wcd9xxx_res->irq) {
		pr_debug("%s: irq driver is not yet initialized\n", __func__);
	...
	ret = request_threaded_irq(wcd9xxx_res->irq, NULL, wcd9xxx_irq_thread,
				   ...
				   "wcd9xxx", wcd9xxx_res);
	if (ret != 0)
		dev_dbg(wcd9xxx_res->dev, "Failed to request IRQ %d: %d\n",
			wcd9xxx_res->irq, ret);
	else {
...
static int phyirq_to_virq(struct wcd9xxx_core_resource *wcd9xxx_res, int offset)
{
	...
	if (!data) {
		pr_debug("%s: not registered to interrupt controller\n",
			__func__);
		return -EINVAL;
	}
...
static int wcd9xxx_irq_probe(struct platform_device *pdev)
{
	...
	if (!gpio_is_valid(irq) && dir_apps_irq < 0) {
		dev_dbg(&pdev->dev, "TLMM connect gpio not found\n");
		return -EPROBE_DEFER;
	}
	if (dir_apps_irq > 0) {
		irq = dir_apps_irq;
		...
		irq = gpio_to_irq(irq);
		if (irq < 0) {
			dev_dbg(&pdev->dev, "Unable to configure irq\n");
			return irq;
		}
	}
