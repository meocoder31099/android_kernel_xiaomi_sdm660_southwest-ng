				  ...
				  unsigned int cmd, void *arg)
{
	...
	case AUDIO_START: {
		...
		if (rc < 0) {
			pr_debug("%s:session id %d: cmd amrwb media format block failed\n",
				__func__, audio->ac->session);
			break;
		}
				  ...
				  unsigned int cmd, unsigned long arg)
{
	...
	case AUDIO_GET_AMRWB_ENC_CONFIG: {
		if (copy_to_user((void *)arg, audio->enc_cfg,
			...
			pr_debug("%s: copy_to_user for AUDIO_GET_AMRWB_ENC_CONFIG failed\n",
				__func__);
			rc = -EFAULT;
		break;
	}
	case AUDIO_SET_AMRWB_ENC_CONFIG: {
		...
		if (copy_from_user(&cfg, (void *) arg,
				sizeof(cfg))) {
			pr_debug("%s: copy_from_user for AUDIO_SET_AMRWB_ENC_CONFIG failed\n",
				__func__);
			rc = -EFAULT;
			break;
		}
				  ...
				  unsigned int cmd, unsigned long arg)
{
	...
	case AUDIO_GET_AMRWB_ENC_CONFIG_32: {
		...
		if (copy_to_user((void *)arg, &amrwb_config_32,
			...
			pr_debug("%s: copy_to_user for AUDIO_GET_AMRWB_ENC_CONFIG_32 failed\n",
				__func__);
			rc = -EFAULT;
		}
		break;
	...
	case AUDIO_SET_AMRWB_ENC_CONFIG_32: {
		...
		if (copy_from_user(&cfg_32, (void *) arg,
				sizeof(cfg_32))) {
			pr_debug("%s: copy_from_user for AUDIO_SET_AMRWB_ENC_CONFIG_32 failed\n",
				__func__);
			rc = -EFAULT;
			break;
		}
...
static int amrwb_in_open(struct inode *inode, struct file *file)
{
	...
	if (!audio->ac) {
		pr_debug("%s:audio[%pK]: Could not allocate memory for audio client\n",
			__func__, audio);
		kfree(audio->enc_cfg);
		kfree(audio);
		return -ENOMEM;
