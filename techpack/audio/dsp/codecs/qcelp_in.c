				...
				unsigned int cmd, unsigned long arg)
{
	...
	case AUDIO_START: {
		...
		if (rc < 0) {
			pr_debug("%s:session id %d: cmd qcelp media format block failed\n",
					__func__, audio->ac->session);
			break;
		}
				...
				unsigned int cmd, unsigned long arg)
{
	...
	case AUDIO_SET_QCELP_ENC_CONFIG: {
		...
		if (rc)
			pr_debug("%s:AUDIO_SET_QCELP_ENC_CONFIG failed. Rc= %d\n",
				__func__, rc);
		break;
	}
	default:
				...
				unsigned int cmd, unsigned long arg)
{
	...
	case AUDIO_GET_QCELP_ENC_CONFIG_32: {
		...
		if (copy_to_user((void *)arg, &cfg_32,
			sizeof(cfg_32))) {
			pr_debug("%s: copy_to_user for AUDIO_GET_QCELP_ENC_CONFIG_32 failed\n",
				__func__);
			rc = -EFAULT;
}
		break;
	...
	case AUDIO_SET_QCELP_ENC_CONFIG_32: {
		...
		if (copy_from_user(&cfg_32, (void *) arg,
				sizeof(cfg_32))) {
			pr_debug("%s: copy_from_user for AUDIO_SET_QCELP_ENC_CONFIG_32 failed\n",
				__func__);
			rc = -EFAULT;
			break;
		}
