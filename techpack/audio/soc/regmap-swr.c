				...
				const void *val, size_t val_len)
{
	...
	if (map == NULL) {
		dev_dbg(dev, "%s: regmap is NULL\n", __func__);
		return -EINVAL;
	}
	addr_bytes = map->format.reg_bytes;
	if (swr == NULL) {
		dev_dbg(dev, "%s: swr device is NULL\n", __func__);
		return -EINVAL;
	}
	if (reg_size != addr_bytes) {
		dev_dbg(dev, "%s: reg size %zd bytes not supported\n",
			__func__, reg_size);
		return -EINVAL;
	}
	reg_addr = *(u16 *)reg;
	...
	for (i = 0; i < (val_len / val_bytes); i++) {
		...
		if (ret < 0) {
			dev_dbg(dev, "%s: write reg 0x%x failed, err %d\n",
					  ...
					  size_t count)
{
	...
	if (addr_bytes + val_bytes + pad_bytes == 0) {
		dev_dbg(dev, "%s: sum of addr, value and pad is 0\n", __func__);
		return -EINVAL;
	}
	...
	if (ret)
		dev_dbg(dev, "%s: multi reg write failed\n", __func__);
			...
			void *val, size_t val_size)
{
	...
	if (ret < 0)
		dev_dbg(dev, "%s: codec reg 0x%x read failed %d\n",
