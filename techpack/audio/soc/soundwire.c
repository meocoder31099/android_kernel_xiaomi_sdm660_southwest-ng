				 ...
				 struct swr_boardinfo const *info)
{
	...
	if (result) {
		dev_dbg(&master->dev, "device [%s] register failed err %d\n",
			swr->name, result);
		goto err_out;
	}
...
int of_register_swr_devices(struct swr_master *master)
{
	...
	for_each_available_child_of_node(master->dev.of_node, node) {
		...
		if (of_modalias_node(node, info.name, sizeof(info.name)) < 0) {
			dev_dbg(&master->dev, "of_swr:modalias failure %s\n",
				node->full_name);
			continue;
		}
		if (of_property_read_u64(node, "reg", &addr)) {
			dev_dbg(&master->dev, "of_swr:invalid reg %s\n",
				node->full_name);
			continue;
		}
		info.addr = addr;
		...
		if (!swr) {
			dev_dbg(&master->dev, "of_swr: Register failed %s\n",
				node->full_name);
			of_node_put(node);
			master->num_dev--;
			continue;
...
void swr_port_response(struct swr_master *mstr, u8 tid)
{
	...
	if (txn == NULL) {
		dev_dbg(&mstr->dev, "%s: transaction is already NULL\n",
			__func__);
		return;
	}
			...
			u8 *ch_mask, u32 *ch_rate, u8 *num_ch, u8 *port_type)
{
	...
	if (num_port > SWR_MAX_DEV_PORT_NUM) {
		dev_dbg(&master->dev, "%s: num_port %d exceeds max port %d\n",
	...
	if (i >= master->last_tid) {
		if (master->last_tid == 255) {
			mutex_unlock(&master->mlock);
			kfree(txn);
			dev_dbg(&master->dev, "%s Max tid reached\n",
				__func__);
			return -ENOMEM;
		}
		...
		if (!temp_txn) {
			mutex_unlock(&master->mlock);
			kfree(txn);
			dev_dbg(&master->dev, "%s Not able to allocate\n"
				...
				struct swr_boardinfo *bi)
{
	...
	if (!swr)
		dev_dbg(&master->dev, "can't create new device for %s\n",
			bi->swr_slave->name);
}
